...
plt.xlabel('Time from ' +
           str(np.min(real_UVM2['Time (MJD)'])) + 'JD in seconds', fontsize=18)
plt.ylabel('Flux (counts/s)', fontsize=18)
plt.title('Simulated Photometric Flux and UVM2 Light Curve', fontsize=18)
plt.tight_layout()
...
for i in range(len(obs_set_numbers)):
    ...
    ax1.legend()
    ax1.set_xlabel('Time (s) since ' + str(t_begin_isot), fontsize=18)
    ax1.set_ylabel('Flux (counts/s)', fontsize=18)
    ax1.set_title('Obs Set ' + str(obs_set_numbers[i]), fontsize=18)
    ax1.axhline(y= 11.423399925231934, color='r', linestyle='-', label = "Median Photometric Flux")
    ax1.legend()
    ...
    for j, fn in enumerate(grism_filenames):
        ...
        txt = np.loadtxt(
            '/Users/shashankchavali/projects/Python/ResearchProject/zeroth_order_subtracted_files/' + fn)

        ax2.plot(txt[:, 0], txt[:, 1], color=cmap(j), linewidth=0.7, alpha=0.7)
        ax3.plot(txt[:, 0], txt[:, 1], color=cmap(j), linewidth=0.7, alpha=0.7)
        ea_interp = np.interp(txt[:, 0], wave_ea, ea)
        ax4.plot(txt[:, 0], ea_interp*txt[:, 1],
                 color=cmap(j), linewidth=0.7, alpha=0.7)

    ax2.set_ylim([-3e-14, 2e-13])
    ax2.set_xlim([1750, 4000])
    ax2.set_xlabel('Wavelength (\AA)', fontsize=18)
    ax2.set_ylabel(
        'Flux Density (erg cm$^{-2}$ s$^{-1}$ \AA$^{-1}$)', fontsize=18)
    ax2.add_patch(Rectangle((2000, 1.599e-15), 800, 3.591e-15,
                            fill=True, color='r', alpha=0.5, zorder=100, figure=fig))

    ax3.set_ylim([-1e-13, 1.3e-12])
    ax3.set_xlim([4000, 7000])
    ax3.set_xlabel('Wavelength (\AA)', fontsize=18)
    ax3.set_ylabel(
        'Flux Density (erg cm$^{-2}$ s$^{-1}$ \AA$^{-1}$)', fontsize=18)

    ax4.set_xlim([1750, 4000])
    ax4.set_ylim([-1e-13, 6e-13])
    ax4.vlines(2200, -1e-13, 6e-13, color='k', linestyle=':', linewidth=2)
    ax4.set_xlabel('Wavelength (\AA)', fontsize=18)
    ax4.set_ylabel('UVM2 Flux Density (erg s$^{-1}$ \AA$^{-1}$)', fontsize=18)

    fig.tight_layout()

    #plt.xlim([buffer_begin, (t_end-t_begin)*24.*3600.+buffer_end])
    ...
    fig.savefig('/Users/shashankchavali/projects/Python/ResearchProject/obs_set_figures/obs_set_' +
